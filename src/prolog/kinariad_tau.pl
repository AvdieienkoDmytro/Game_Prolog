...
%   - Column ordering: center-first improves alpha-beta cutoffs significantly
%   - Top-level best_move uses alpha-beta directly (no findall over all moves)
%   - win_check uses last-placed position to avoid full board scan
...
% sort_center(++Cols,++Mid,--Sorted)
% Sort columns by distance to center (closest first).
% Multimodality:
...
sort_center(Cols, Mid, Sorted) :-
    findall(D-C,
        ...
        Pairs),
    msort(Pairs, Sorted1),
    pairs_values(Sorted1, Sorted).

% pairs_values(++Pairs,--Values) — extract second elements from key-value pairs
% Multimodality:
%   pairs_values(++,--) — extract values from sorted pairs
pairs_values([], []).
...
check_dir(Board, R, C, Player, K, DR, DC) :-
    ...
    Total is N1 + N2 - 1,   % (R,C) counted in both directions
    Total >= K.
...
% Full board scan — used only at terminal node check in minimax.
% Multimodality:
...
% Heuristic: Player's threat points minus opponent's threat points.
% Multimodality:
...
% sum_list_tau(++List,--Sum) — portable sum (no library(lists) sumlist in Tau)
% Multimodality:
...
% Depth measured in HALF-MOVES (пів-ходи)
% ============================================================
...
% Terminal: opponent already won before this move → large negative for us
...
% Uses alpha-beta at root (not just findall) for early cutoffs.
% Multimodality:
