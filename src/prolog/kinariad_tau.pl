% kinariad_tau.pl — K в ряд (падіння), адаптовано для Tau-Prolog
...
% Мультипризначенність: new_game(++,++,++,++,--) — створення дошки
...
% Мультипризначенність: init_cells(++,++,++,++,++,++) — ініціалізація
...
%   cell_idx(++,++,++,--) — обчислення індексу
%   cell_idx(--,--,++,++) — відновлення координат
cell_idx(R, C, Cols, Idx) :-
    ( var(Idx) ->
        Idx is (R-1)*Cols + C
    ;
...
% Мультипризначенність:
%   get_cell(++,++,++,--) — читання
%   get_cell(++,++,++,+)  — перевірка значення
get_cell(board(_,Cols,_,_,Cells), R, C, Val) :-
    cell_idx(R, C, Cols, Idx),
    nth1(Idx, Cells, Val).

%% set_cell(++Board,++R,++C,++Val,--NewBoard)
% Мультипризначенність: set_cell(++,++,++,++,--) — оновлення дошки
...
% Мультипризначенність: replace_nth1(++,++,++,--) — заміна елемента
...
% Мультипризначенність:
%   drop_piece(++,++,++,--,--) — падіння фішки (основне)
%   drop_piece(++,++,++,--,+)  — з перевіркою рядка посадки
drop_piece(Board, Col, Player, NewBoard, Row) :-
...
% Мультипризначенність: find_bottom(++,++,++,--) — пошук вільного рядка
find_bottom(Board, Col, CurR, Row) :-
    CurR >= 1,
...
% Мультипризначенність: col_available(++,++) — перевірка доступності
col_available(Board, Col) :-
...
% Мультипризначенність: available_cols(++,--) — перелік доступних ходів
available_cols(Board, Cols) :-
...
%% opponent(++P,--Opp)
% Мультипризначенність:
%   opponent(++,--) — визначення противника
%   opponent(--,++) — зворотній напрям
%   opponent(++,+)  — перевірка пари
opponent(x, o).
opponent(o, x).

%% winner(++Board,++R,++C,--Player)
% Мультипризначенність:
%   winner(++,++,++,--) — визначення переможця після ходу
%   winner(++,++,++,+)  — перевірка конкретного гравця
winner(Board, R, C, Player) :-
...
% Мультипризначенність: check_dir(++,++,++,++,++,++,++) — перевірка напрямку
check_dir(Board, R, C, Player, K, DR, DC) :-
...
% Мультипризначенність: count_dir(++,++,++,++,++,++,++,--) — підрахунок
count_dir(Board, R, C, Player, DR, DC, Acc, Count) :-
...
% Мультипризначенність: win_exists(++,++) — перевірка виграшу на дошці
win_exists(Board, Player) :-
...
% Мультипризначенність: eval_board(++,++,--) — евристична оцінка
eval_board(Board, Player, Score) :-
    opponent(Player, Opp),
...
% Мультипризначенність: count_threats(++,++,--) — підрахунок загроз
count_threats(Board, Player, Pts) :-
    Board = board(Rows,Cols,K,_,_),
    findall(P, window_pts(Board,Player,K,Rows,Cols,P), List),
    sum_list(List, Pts).

% Tau-Prolog: sumlist як резервний варіант
sum_list([], 0).
...
%% score_for(++N,--Score) — очки за N власних фішок у вільному вікні
score_for(1, 1).
score_for(2, 10).
score_for(3, 100).
score_for(4, 1000).
...
% ============================================================
% MinMax з Alpha-Beta (глибина у ПІВ-ХОДАХ)
% ============================================================

%% minimax(++Board,++Depth,++Alpha,++Beta,++Player,++IsMax,--Score)
% Мультипризначенність: minimax(++,++,++,++,++,++,--) — оцінка вузла
...
% Мультипризначенність: expand_max(++,++,++,++,++,++,++,++,--) — вузол максимізатора
...
% Мультипризначенність: expand_min(++,++,++,++,++,++,++,++,--) — вузол мінімізатора
...
% Мультипризначенність: best_move(++,++,++,--) — вибір найкращого ходу ШІ
...
%% max_pair(++List,--Best) — знаходить пару з максимальним першим елементом
% Мультипризначенність: max_pair(++,--) — пошук максимуму
max_pair([H], H).
...
% ============================================================
% Bridge predicates — виклики з JavaScript
% ============================================================

%% js_new_game(++Rows,++Cols,++K,++ForbAtom,--BoardAtom)
% Серіалізує дошку у атом для передачі у JS
js_new_game(Rows, Cols, K, ForbAtom, BoardAtom) :-
...
% Result: ok / blocked / full / win(Player) / draw
js_human_move(BoardAtom, Col, PlayerAtom, NewBoardAtom, Result) :-
    term_to_atom(Board, BoardAtom),
...
% Виконує хід ШІ та повертає результат
js_ai_move(BoardAtom, PlayerAtom, Depth, NewBoardAtom, Col, Result) :-
    term_to_atom(Board, BoardAtom),
    term_to_atom(Player, PlayerAtom),
...
% Повертає список клітин як атом
js_get_cells(BoardAtom, CellsAtom) :-
    term_to_atom(board(_,_,_,_,Cells), BoardAtom),
    term_to_atom(Cells, CellsAtom).

%% js_board_info(++BoardAtom,--Rows,--Cols,--K)
% Повертає параметри дошки
js_board_info(BoardAtom, Rows, Cols, K) :-
    term_to_atom(board(Rows,Cols,K,_,_), BoardAtom).

%% js_forbidden(++BoardAtom,--ForbAtom)
% Повертає список заборонених клітин
js_forbidden(BoardAtom, ForbAtom) :-
    term_to_atom(board(_,_,_,Forb,_), BoardAtom),
    term_to_atom(Forb, ForbAtom).
